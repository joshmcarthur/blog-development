...
# Configuration
TUMBLR_DOMAIN =   "http://blog.joshmcarthur.com"
WRITE_DIRECTORY = "_posts"
IMAGE_DIRECTORY = "../public/images/assets/"
TEMP_DIRECTORY = "tmp"
LAYOUT = "default"
...
while posts_returned != 0
  ...
  open(path) do |xml|
    ...
    posts.each do |post_tag|
      ...
      filename = "#{date.strftime("%Y-%m-%d")}-#{slug}"
      ...
      temp_html_file = File.new("#{TEMP_DIRECTORY}/#{filename}", "w+")
...
#{`html2text "#{TEMP_DIRECTORY}/#{filename}"`}
EOPOST
      ...
      file = File.new("#{WRITE_DIRECTORY}/#{filename}.markdown", "w+")
